  ...
  <div>
    <div class="collapsepanel" v-if="pageReady">
      <div
        ...
        :key="index"
        v-show="index>=pageSize*(pageNum-1) && index<pageSize*pageNum"
      >
        <div
          class="collapsepanel-title"
        ...
        <el-row :gutter="20" v-show="index == activeIndex">
          ...
          <el-col :span="9" class="collapsepanel-tools">
            <div class="grid-content collaspe-btn clearfix">
              <el-button @click="updateVisibleShow(index, item.storageId)" type="primary">修改</el-button>
              <el-button
                type="primary"
                @click="setConnect"
                :id="item.storageId"
                :disabled="item.storageId == seledId"
              >关联</el-button>
              <el-button
                type="primary"
                @click="setDelete"
    ...
    <div class="clearfix">
      <el-button type="primary" @click="addVisibleShow">新增</el-button>
    </div>
    <!--新增功能-->
    <hdfs-add :msg="1" v-if="addVisible" ref="addOrUpdate" @refresh="refresh()"></hdfs-add>
    <!--修改功能-->
    <hdfs-edit
      v-if="updateVisible"
      ref="addOrEdit"
      :indexEq="updateIndex"
      :ownId="updateStorageId"
      @refresh="refreshs(updateIndex)"
    ></hdfs-edit>
  </div>
</template>
<script>
...
export default {
  ...
  watch: {
    settingList: {
      ...
      handler: function(newVal, oldVal) {
        ...
        if (this.isDelPage != -1) {
          //删除之后跳转到操作页面并不展示内容
          if(this.settingList.list.length%5==0){
            this.activeIndex = 0;
            this.currentPage = 1;
            this.pageNum = 1;
          } else {
          ...
          this.currentPage = 1;
          this.pageNum = 1;
        } else if (this.isEditIndex != -1) {
          //修改flag，修改之后跳转到当前页，当前项
          ...
          for (var i = 0; i < this.settingList.list.length; i++) {
            if (
              this.settingList.list[i].storageId == this.settingList.seledId
            ) {
              this.activeIndex = i;
  ...
  methods: {
    ...
    connectAjax() {
      ...
      this.$ajax
        .get(window.ENV.API_DACM + "/caccesssysRelationWorkInfo/operate", {
          params: {
            nodeId: 2,
            storageId: storageId
          }
    ...
    delectAjax(id) {
      var _self = this;
      this.$ajax
        .get(window.ENV.API_DACM + "/caccesssysRelationWorkInfo/delete", {
          params: {
            pid: 2,
            id: id
          }
    ...
    addVisibleShow() {
      ...
      this.$nextTick(() => {
        this.$refs.addOrUpdate.addDiagVisible();
      });
    },
    updateVisibleShow(index, id) {
      this.updateVisible = true;
      this.updateIndex = index;
      this.updateStorageId = id;
      this.$nextTick(() => {
        this.$refs.addOrEdit.updateDiagVisible();
      });
    }
  },
  components: {
