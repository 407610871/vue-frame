  ...
  <div class="taskMDialog" style="padding-bottom:15px;">
    <el-dialog :title="reqObj.taskName" :close-on-click-modal="false" :visible.sync="showInnerDialog" @closed="closeDia" width="75%">
      ...
      <el-form label-width="150px" class="demo-ruleForm">
        <span v-show="taskBaseInfo.status==1||taskBaseInfo.status==2||(taskBaseInfo.status==5&&reqObj.startTime!=''&&reqObj.startTime!='undefined'&&reqObj.startTime!=undefined)" style="float:right;">当前状态:
          <el-select
            v-model="flagDesc"
            :disabled="loading3"
            placeholder="请选择"
        ...
        <div class="daiInfo dockInfo" v-loading="loading2||isCheckLoading">
          ...
          <div class="daiInfo-box clearfix">
            ...
            <el-col :span="4" :class="{bank:!((reqObj.status==1||reqObj.status==2||reqObj.status==4)&&reqObj.isPeriod!=0&&reqObj.ftpIsDelete!='true')}">
              {{((reqObj.status==1||reqObj.status==2||reqObj.status==4)&&reqObj.isPeriod!=0&&reqObj.ftpIsDelete!='true')?' ':'bank'}}
              <el-button v-show="(reqObj.status==1||reqObj.status==2||reqObj.status==4)&&reqObj.isPeriod!=0&&reqObj.ftpIsDelete!='true'" type="primary" size="small" @click="checkData">数据核验</el-button>
            </el-col>
            <el-col :span="10">
