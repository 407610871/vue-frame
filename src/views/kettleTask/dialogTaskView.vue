    ...
    >
      ...
      <el-form label-width="100px">
        ...
        <div class="daiInfo dockInfo">
          <div class="clearfix">
            ...
            <el-col :span="12" class="info-right" v-loading="loading2">
              <div class="taskStatus">
                <span style="width:80px;">处理任务:</span>
                <span :title="reqObj.taskName">{{reqObj.taskName}}</span>
                <span v-if="reqObj.status=='create'" class="span-text">新建</span>
                <span v-if="reqObj.status=='Running'" class="span-text">运行</span>
                <span v-if="reqObj.status=='Paused'" class="span-text">暂停</span>
                <span v-if="reqObj.status=='Finished (with errors)'" class="span-text">失败</span>
                <span v-if="reqObj.status=='Finished'" class="span-text">完成</span>
              </div>
              <div>
                <span>输入：</span>
                <span>{{taskBaseInfo.input}}</span>
              </div>
              <div style="height:78px">
                ...
                <div class="steps">
                  <P
                    ...
                    v-for="(item,index) in taskBaseInfo.steps"
                    :key="item.jobEntryType"
                  >步骤{{index+1}}：{{item.jobEntryName}}</P>
                </div>
              </div>
              <div>
                <span>输出：</span>
                <span>{{taskBaseInfo.output}}</span>
              </div>
              <div>
                <span>创建时间：</span>
                <span>{{reqObj.createTime}}</span>
              </div>
              <div>
                <span>任务开始时间：</span>
          ...
          <el-button class="button-log" type="primary" @click="queryLogs()">日志</el-button>
          <!-- 四个日志tab 开始 -->
          <div class="daiInfo-tabs">
            <div type="border-card" style="height:265px;">
              <div class="dataCheck-tab" v-loading="loading3">
                <textarea
                  v-show="taskBaseInfo.log!=''"
                  v-model="taskBaseInfo.log"
                  name
                  id
                  cols="30"
                  rows="12"
...
export default {
  ...
  created() {
    //时间格式转换
    this.reqObj.createTime = formatDateTime(this.reqObj.createTime);
    //时间格式转换
    this.reqObj.startTime = formatDateTime(this.reqObj.startTime);
    //时间格式转换
    this.reqObj.endTime = formatDateTime(this.reqObj.endTime);
    //查询流程图
  ...
  methods: {
    ...
    getFlowChartPath() {
      ...
      that.$ajax
        .post(
          that.httpUrlOld + "manager/govern/getFlowChartPath",
          reqData.params
        )
        .then(function(res) {
          if (
            ...
            that.doMsg(
              "“manager/govern/getFlowChartPath”服务响应为空！",
              "error"
            );
          } else if (res.data.code != "200" && res.data.code != "0000") {
            that.doMsg(
              "“manager/govern/getFlowChartPath”" + res.data.message,
              "error"
            );
          } else {
            that.taskBaseInfo.flowChartPath = res.data.data.path;
            //图片url换行符需要被识别，所以在此经过textarea转换一下，解决方式有点low，有更好解决办法的欢迎更改
            document.getElementById("flowChartPath").innerHTML =
              that.taskBaseInfo.flowChartPath;
            that.src1 =
    ...
    getInputAndOutput() {
      ...
      that.$ajax
        .post(
          that.httpUrlOld + "manager/govern/getInputAndOutput",
          reqData.params
        )
        .then(function(res) {
          if (
            ...
            that.doMsg(
              "“manager/govern/getInputAndOutput”服务响应为空！",
              "error"
            );
          } else if (res.data.code != "200" && res.data.code != "0000") {
            that.doMsg(
              "“manager/govern/getInputAndOutput”" + res.data.message,
              "error"
            );
          } else {
    ...
    getTransInfo() {
      ...
      that.$ajax
        .post(that.httpUrlOld + "manager/govern/getTransInfo", reqData.params)
        .then(function(res) {
          if (
            res.data.code == undefined ||
            res.data.code == null ||
            ...
            that.doMsg(
              "“/manager/govern/getTransInfo”" + res.data.message,
              "error"
            );
          } else {
    ...
    queryLogs() {
      ...
      that.$ajax
        .post(that.httpUrlOld + "manager/govern/queryLogs", reqData.params)
        .then(function(res) {
          if (
            res.data.code == undefined ||
            res.data.code == null ||
            ...
            that.doMsg(
              "“/manager/govern/queryLogs”" + res.data.message,
              "error"
            );
          } else {
